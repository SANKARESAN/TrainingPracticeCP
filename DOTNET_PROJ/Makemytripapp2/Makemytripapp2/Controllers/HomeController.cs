...
namespace Makemytripapp.Controllers
{
    public class HomeController : Controller
    {
        ...
        public async Task<IActionResult> GetReservation(int id)
        {
            ...
            {
                using (var response = await httpClient.GetAsync("http://localhost:5161/api/Reservation/" + id))
                {
        ...
        public async Task<IActionResult> AddReservation(Reservation reservation)
        {
            ...
            using (var httpClient = new HttpClient())
            {
                ...
                using (var response = await httpClient.PostAsync("http://localhost:5161/api/Reservation", content))
                {
                    string apiResponse = await response.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> UpdateReservation(Reservation reservation)
        {
            ...
            using (var httpClient = new HttpClient())
            {
                ...
                using (var response = await httpClient.PutAsync("https://localhost:5161/api/Reservation", content))
                {
                    string apiResponse = await response.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> DeleteReservation(int ReservationId)
        {
            using (var httpClient = new HttpClient())
            {
                using (var response = await httpClient.DeleteAsync("http://localhost:5161/api/Reservation/" + ReservationId))
                {
                    string apiResponse = await response.Content.ReadAsStringAsync();
                }
            }
        ...
        */
        ...
        public async Task<IActionResult> Index()
        {
            ...
            using (var httpClient = new HttpClient())
            {
                using (var response = await httpClient.GetAsync("http://localhost:5161/api/Reservation"))
                {
                    string apiResponse = await response.Content.ReadAsStringAsync();
